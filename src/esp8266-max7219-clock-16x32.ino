...
String newHostname = "Clock16x32LED";
...
#define PAUSE_TIME_LL 30000 //30s for month/day long pause
...
void setup() {
  ...
  P.begin(4); // 4 zones
  P.setZone(0, 1, 3);  //1110 <- Zone 0  for Time display
  P.setZone(1, 0, 0);  //0001 <- Zone 1  for Second display
  P.setZone(2, 5, 7);  //1110 <- Zone 2  for Date display
  P.setZone(3, 4, 4);  //0001 <- Zone 3  for WeekDay display
...
void loop() {
  ...
  //Zone 3 Day of the Week 
  if (P.getZoneStatus(3)) {
    P.displayZoneText(3, bufW, PA_CENTER, SPEED_TIME, 0, PA_PRINT, PA_NO_EFFECT);
    P.displayReset(3);
  }
  ...
  if (P.getZoneStatus(2)) {
    ...
    switch (dsw) {
      case 0: // year
        P.displayZoneText(2, bufY, PA_CENTER, SPEED_TIME, PAUSE_TIME_S, PA_SCROLL_LEFT, PA_SCROLL_LEFT);
        dsw++;
        break;
      case 1: // month/day
        P.displayZoneText(2, bufD, PA_CENTER, SPEED_TIME, PAUSE_TIME_LL, PA_SCROLL_LEFT, PA_SCROLL_LEFT);
        dsw = 0;
        break;
      default:
        break;
  ...
  if (P.getZoneStatus(1)) {
    P.displayZoneText(1, bufS, PA_CENTER, SPEED_TIME, 0, PA_PRINT, PA_NO_EFFECT);
    P.displayReset(1);
  }
